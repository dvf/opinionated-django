...
Standard Django signals are synchronous and unreliable — receiver failures propagate to the sender, there's no delivery guarantee if the process crashes after commit, and there's no retry. This project uses the [reliable signals pattern](https://hakibenita.com/django-reliable-signals) by Haki Benita, adapted to use Celery instead of Django's Tasks framework.

### How It Works

Signal receiver tasks are enqueued **inside the same database transaction** as the business operation. If the transaction rolls back, the tasks roll back too. If it commits, the tasks are guaranteed to be in the queue.

```
┌─────────────────────────────────┐
│  Database Transaction           │
│                                 │
│  1. Business write (e.g. Order) │
...
Each app defines its signals in a `signals.py` module:
...
Use `send_reliable()` inside a `transaction.atomic()` block in the service layer. Arguments must be JSON-serializable — pass entity IDs, not model instances:
...
Receivers are registered with the standard `@receiver` decorator. They execute asynchronously via Celery.

**Receivers MUST be idempotent.** The system guarantees at-least-once delivery, which means a receiver may execute more than once in edge cases (e.g. worker crash after execution but before acknowledgement). Design every receiver so that running it twice with the same arguments produces the same result.
...
def on_order_created(order_id: str, **kwargs):
    # Idempotent: check if confirmation already sent before sending
...
- Failed receivers are retried automatically with exponential backoff
