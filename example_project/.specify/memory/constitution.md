<!--
Sync Impact Report
- Version change: future-django Constitution v1.2.0 -> v1.3.0
- List of modified principles:
  ...
  - PRINCIPLE_7: Stripe-style Prefixed ULID Identifiers (unchanged)
  - PRINCIPLE_8: Reliable Signals (Celery) (NEW)
  - PRINCIPLE_9: Type Stubs for Django Extensions (.pyi) (renumbered from VIII)
- Added sections: Technology Stack expanded
...
### VIII. Reliable Signals (Celery)
Side-effects triggered by business operations (notifications, cache invalidation, analytics) MUST use reliable signals, not standard Django signals. Signal receiver tasks are enqueued inside the same database transaction as the business write, guaranteeing delivery only on commit and automatic rollback on failure. Celery executes receivers asynchronously with automatic retry. All signal arguments MUST be JSON-serializable (pass entity IDs, never model instances). All receivers MUST be idempotent — the system guarantees at-least-once delivery, not exactly-once. This pattern is based on [Haki Benita's Reliable Signals](https://hakibenita.com/django-reliable-signals).

### IX. Type Stubs for Django Extensions (.pyi)
...
- **IDs**: python-ulid (Stripe-style prefixed ULIDs)
- **Task Queue**: Celery (reliable signals)
...
**Version**: 1.3.0 | **Ratified**: 2026-03-28 | **Last Amended**: 2026-04-07
